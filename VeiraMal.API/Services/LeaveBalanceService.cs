...
namespace VeiraMal.API.Services
{
    public class LeaveBalanceService : ILeaveBalanceService
    {
        ...
        {
            var list = await ParseLeaveBalanceAsync(file);
            ...
            return $"{list.Count} LeaveBalance records uploaded (old data replaced).";
        }

        private async Task<List<LeaveBalance>> ParseLeaveBalanceAsync(IFormFile file)
        {
            ...
            if (file.FileName.EndsWith(".csv", StringComparison.OrdinalIgnoreCase))
            {
                ...
                _ = await sr.ReadLineAsync(); // skip header
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    decimal alBalance = SafeDecimal(cols, 3);
        ...
        private static string SafeString(List<string> cols, int idx) =>
            (idx < cols.Count) ? cols[idx].Trim() : string.Empty;
        ...
        private static DateTime SafeParseDate(List<string> cols, int idx)
        {
            ...
            if (double.TryParse(s, out var oaDate))
